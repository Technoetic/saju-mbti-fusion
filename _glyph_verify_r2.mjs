...
const LOG = (s) => { console.log(s); fs.appendFileSync('_glyph_r2.out', s + '\n'); };
try { fs.unlinkSync('_glyph_r2.out'); } catch {}

const b = await chromium.launch();
const ctx = await b.newContext({ viewport: { width: 430, height: 932 }, deviceScaleFactor: 2 });
const p = await ctx.newPage();
p.on('console', m => { try { fs.appendFileSync('_glyph_r2.out', '[console] ' + m.text() + '\n'); } catch {} });
...
let ok = false;
for (let i = 0; i < 60; i++) {
  if (await p.evaluate(() => !!document.querySelector('.webtoon-container'))) { ok = true; break; }
  await p.waitForTimeout(2000);
}
LOG('STEP: webtoon ready=' + ok);
...
const result = await p.evaluate(async () => {
  ...
  for (let pi = 0; pi < pages.length; pi++) {
    ...
    if (!img || !img.naturalWidth) { out.push({ pageIdx: pi, slots: [], err: 'no-img' }); continue; }
    const cv = document.createElement('canvas');
    cv.width = img.naturalWidth; cv.height = img.naturalHeight;
    const cx = cv.getContext('2d');
    cx.drawImage(img, 0, 0);
    let d;
    try { d = cx.getImageData(0, 0, cv.width, cv.height).data; } catch (e) { out.push({ pageIdx: pi, slots: [], err: 'tainted:' + e.message }); continue; }
    ...
    const slots = Array.from(pageEl.querySelectorAll('.webtoon-slot')).map(s => {
      ...
      const sample = (fx, fy) => {
        ...
        if (px < 0 || px >= W || py < 0 || py >= H) return { isWhite: false, isBrown: false };
        const i = (py * W + px) * 4;
      ...
      for (let yy = 0; yy < 8; yy++) {
        for (let xx = 0; xx < 8; xx++) grid.push(sample((xx + 0.5) / 8, (yy + 0.5) / 8));
      }
...
LOG('GLYPH_RESULT:' + JSON.stringify(result));
await b.close();
LOG('STEP: done');
