...
p.on('response', async r => {
  if (r.url().includes('/api/saju/webtoon')) {
    networkLog.push({ url: r.url(), status: r.status(), len: (await r.text().catch(()=>'')).length });
  }
});
p.on('pageerror', e => networkLog.push({ error: e.message }));
...
// 풀이 + nano-banana 합쳐 최대 240초 대기
let bananaPages = 0;
for (let i=0; i<120; i++) {
  bananaPages = await p.evaluate(() => document.querySelectorAll('.webtoon-page-banana').length);
  if (bananaPages >= 1) break;
  await p.waitForTimeout(2000);
}
console.log('banana pages rendered:', bananaPages);
console.log('network log:', JSON.stringify(networkLog, null, 2));

await p.waitForTimeout(3000);
await p.screenshot({ path: '_nano_e2e.png', fullPage: false });
// 첫 페이지만 따로
const first = await p.$('.webtoon-page-banana:first-of-type');
if (first) await first.screenshot({ path: '_nano_first_page.png' });
await b.close();
