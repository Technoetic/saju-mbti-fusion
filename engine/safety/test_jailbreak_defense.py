"""engine.safety.jailbreak_defense — §5.2.4 적대적 프롬프트 회귀 테스트."""
...
def test_clean_input_no_hits():
    from engine.safety.jailbreak_defense import detect_jailbreak, is_jailbreak_attempt
    benign = [
        "이번 달 운세가 어떤가요?",
        "제 얼굴 보고 결혼운 알려주세요",
...
def test_multiple_categories_detected():
    from engine.safety.jailbreak_defense import detect_jailbreak
...
def test_rejection_text_unknown_category_returns_empty():
    from engine.safety.jailbreak_defense import get_rejection_text
...
def test_build_response_clean_input():
    from engine.safety.jailbreak_defense import build_jailbreak_response
