"""engine.safety.rollback_trigger — §7.3.2.1 자동 롤백 회귀 테스트."""
...
def test_auto_policy_for_critical_events():
    from engine.safety.rollback_trigger import classify_rollback_policy, AUTO
...
def test_approval_policy_for_performance_events():
    from engine.safety.rollback_trigger import classify_rollback_policy, APPROVAL
...
def test_never_policy_for_quality_events():
    from engine.safety.rollback_trigger import classify_rollback_policy, NEVER
...
def test_revert_command_format():
    from engine.safety.rollback_trigger import build_revert_command
...
def test_decide_rollback_auto_event():
    from engine.safety.rollback_trigger import decide_rollback, AUTO
...
def test_decide_rollback_approval_event_has_window():
    from engine.safety.rollback_trigger import decide_rollback, APPROVAL
...
def test_decide_rollback_never_event_still_returns_command():
    ...
    from engine.safety.rollback_trigger import decide_rollback, NEVER
...
def test_to_alert_payload_has_required_fields():
    from engine.safety.rollback_trigger import decide_rollback, to_alert_payload
