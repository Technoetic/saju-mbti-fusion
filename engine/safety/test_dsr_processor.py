"""engine.safety.dsr_processor — §10 DSR 처리 회귀 테스트."""
...
def _mk(
    ...
    from engine.safety.dsr_processor import DSRRequest
    return DSRRequest(
        right_key=right_key,
...
def _setup_cache(tmp_path: Path, subject_id: str) -> Path:
    ...
    from engine.safety.dsr_processor import _subject_hash
...
def test_ingest_valid_request_ok():
    from engine.safety.dsr_processor import ingest_dsr
...
def test_plan_erase_includes_delete_actions(tmp_path):
    from engine.safety.dsr_processor import plan_dsr, DSR_STATUS_PLANNED
...
def test_plan_access_uses_list_action(tmp_path):
    from engine.safety.dsr_processor import plan_dsr
    cache = _setup_cache(tmp_path, "user-abc")
...
def test_plan_invalid_request_returns_rejected(tmp_path):
    from engine.safety.dsr_processor import plan_dsr, DSR_STATUS_REJECTED
...
def test_execute_erase_deletes_only_matching_files(tmp_path):
    from engine.safety.dsr_processor import plan_dsr, execute_dsr
    cache = _setup_cache(tmp_path, "user-abc")
    p = plan_dsr(_mk(right_key="erase"), cache_dir=cache)
...
def test_audit_record_has_no_raw_subject_id(tmp_path):
    ...
    from engine.safety.dsr_processor import plan_dsr, execute_dsr, build_audit_record
    cache = _setup_cache(tmp_path, "user-abc")
...
def test_process_dsr_full_flow_erase(tmp_path):
    from engine.safety.dsr_processor import process_dsr
    cache = _setup_cache(tmp_path, "user-abc")
