"""engine.safety.request_pipeline — §7.2.22 통합 파이프라인 회귀 테스트."""
...
def test_preflight_clean_input_allowed():
    from engine.safety.request_pipeline import preflight
    d = preflight(
        image_b64="dummy",
...
def test_preflight_rate_limited_blocks():
    from engine.safety.request_pipeline import preflight, BLOCK_RATE_LIMITED
    from engine.safety.rate_limiter import RateLimiter, RateLimitConfig
...
def test_preflight_cost_exhausted_blocks():
    from engine.safety.request_pipeline import preflight, BLOCK_COST_EXHAUSTED
...
def test_preflight_empty_after_sanitize_blocks():
    from engine.safety.request_pipeline import preflight, BLOCK_INPUT_EMPTY
    d = preflight(
        image_b64="x",
...
def test_preflight_jailbreak_blocks():
    from engine.safety.request_pipeline import preflight, BLOCK_JAILBREAK
    d = preflight(
        image_b64="x",
...
def test_preflight_idempotency_first_claim():
    from engine.safety.request_pipeline import preflight
    from engine.safety.idempotency_key import IdempotencyManager
...
def test_to_trace_event_fields():
    from engine.safety.request_pipeline import preflight, to_trace_event
