"""engine.safety.input_guards.canary_guard — §7.3.6 카나리 가드 회귀 테스트."""
...
def _mk_metrics(
    ...
    from engine.safety.input_guards.canary_guard import CanaryMetrics
...
def test_canary_stages_increasing():
    from engine.safety.input_guards.canary_guard import CANARY_STAGES
...
def test_canary_thresholds_present():
    from engine.safety.input_guards.canary_guard import CANARY_THRESHOLDS
...
def test_sufficient_sample_thresholds():
    from engine.safety.input_guards.canary_guard import has_sufficient_sample
...
def test_crisis_block_failed_triggers_rollback():
    from engine.safety.input_guards.canary_guard import decide_canary, DECISION_ROLLBACK
...
def test_err_rate_delta_over_threshold_holds():
    from engine.safety.input_guards.canary_guard import decide_canary, DECISION_HOLD
...
def test_all_checks_pass_promotes():
    from engine.safety.input_guards.canary_guard import decide_canary, DECISION_PROMOTE
    c = _mk_metrics()
    b = _mk_metrics()
...
def test_promote_advances_through_stages():
    from engine.safety.input_guards.canary_guard import decide_canary
    c = _mk_metrics()
    b = _mk_metrics()
    # 1% → 5% → 25% → 100%
...
def test_metrics_from_slo_report_maps_fields():
    from engine.safety.input_guards.canary_guard import metrics_from_slo_report
    slo = {
        "request_count": 500,
        "err_rate": 0.15,
...
def test_to_alert_payload_rollback():
    from engine.safety.input_guards.canary_guard import decide_canary, to_alert_payload
    c = _mk_metrics(crisis_block_failed=1)
    b = _mk_metrics()
    d = decide_canary(canary_metrics=c, baseline_metrics=b,
