"""engine.safety.slo.latency_audit — §7.2.24 latency 감사 회귀 테스트."""
...
def test_new_sample_sets_started_at():
    from engine.safety.slo.latency_audit import new_sample
...
def test_record_step_stores_duration():
    from engine.safety.slo.latency_audit import new_sample, record_step, STEP_LLM_CALL
    s = new_sample()
...
def test_finalize_sets_finished_at():
    from engine.safety.slo.latency_audit import new_sample, finalize
    s = new_sample()
    finalize(s)
...
def test_measure_records_elapsed():
    from engine.safety.slo.latency_audit import new_sample, measure, STEP_LLM_CALL
    s = new_sample()
...
def test_aggregate_step_p95_empty():
    from engine.safety.slo.latency_audit import aggregate_step_p95, STEP_LLM_CALL
...
def test_aggregate_total_p95_empty():
    from engine.safety.slo.latency_audit import aggregate_total_p95
