"""engine.safety.crisis.alert_router — §14.3 알람 라우터 회귀 테스트."""
...
def test_classify_slo_violation_crisis_rate_is_p0():
    from engine.safety.crisis.alert_router import classify_slo_violation, P0
    assert classify_slo_violation("crisis_rate=0.080 > 0.05") == P0


def test_classify_slo_violation_p95_is_p1():
    from engine.safety.crisis.alert_router import classify_slo_violation, P1
...
def test_classify_slo_violation_cache_is_p2():
    from engine.safety.crisis.alert_router import classify_slo_violation, P2
    assert classify_slo_violation("cache_hit_rate=0.05 < 0.10") == P2


def test_classify_event_crisis_block_failed_is_p0():
    from engine.safety.crisis.alert_router import classify_event, P0
...
def test_classify_event_golden_set_regression_severity_by_failures():
    from engine.safety.crisis.alert_router import classify_event, P1, P2
...
def test_classify_event_data_governance_is_p2():
    from engine.safety.crisis.alert_router import classify_event, P2
    assert classify_event("data_governance_violation") == P2


def test_classify_event_unknown_falls_to_p3():
    from engine.safety.crisis.alert_router import classify_event, P3
...
def test_debouncer_first_send_passes():
    from engine.safety.crisis.alert_router import Debouncer
...
def test_route_alert_p0_uses_pagerduty_and_slack_incidents():
    from engine.safety.crisis.alert_router import AlertEvent, route_alert, P0
...
def test_route_alert_p1_uses_slack_incidents_only():
    from engine.safety.crisis.alert_router import AlertEvent, route_alert, P1
...
def test_route_alert_p2_uses_ops_daily():
    from engine.safety.crisis.alert_router import AlertEvent, route_alert, P2
...
def test_route_alert_p3_uses_email():
    from engine.safety.crisis.alert_router import AlertEvent, route_alert, P3
...
def test_route_alert_debounced_returns_suppressed():
    from engine.safety.crisis.alert_router import AlertEvent, route_alert, P1, Debouncer
    d = Debouncer(window_sec=300)
...
def test_slack_message_has_block_kit_structure():
    from engine.safety.crisis.alert_router import AlertEvent, build_slack_message, P0
...
def test_pagerduty_payload_dedup_key_is_stable():
    from engine.safety.crisis.alert_router import AlertEvent, build_pagerduty_event, P0
    e = AlertEvent(name="slo_violation:crisis_rate", severity=P0, title="X")
...
def test_alerts_from_slo_report_creates_events_per_violation():
    from engine.safety.crisis.alert_router import alerts_from_slo_report
    slo = {
        "request_count": 100,
        "crisis_rate": 0.08,
        "err_rate": 0.25,
