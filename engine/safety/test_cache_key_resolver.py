"""engine.safety.cache_key_resolver — §7.2.23 캐시 키 회귀 테스트."""
...
def test_same_input_same_key():
    from engine.safety.cache_key_resolver import resolve_cache_key
...
def test_key_length():
    from engine.safety.cache_key_resolver import resolve_cache_key, CACHE_KEY_LENGTH
...
def test_invalidates_returns_true_when_different():
    from engine.safety.cache_key_resolver import invalidates_on_prompt_change
...
def test_trace_event_omits_raw_pii():
    ...
    from engine.safety.cache_key_resolver import resolve_cache_key, to_trace_event
    k = resolve_cache_key(**_base_kwargs())
    e = to_trace_event(k)
