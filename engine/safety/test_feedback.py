"""engine.safety.feedback — §7.2.7 사용자 피드백 수집기 단위 테스트.
...
def test_reading_hash_deterministic():
    """같은 본문 → 같은 키."""
    from engine.safety.feedback import reading_hash
...
def test_record_feedback_basic():
    ...
    from engine.safety.feedback import record_feedback, get_reading_counts
    text = "허허, 시험이로세."
...
def test_record_feedback_invalid_kind():
    """무효 kind 무시."""
    from engine.safety.feedback import record_feedback
...
def test_get_reading_counts_unknown_returns_none():
    """없는 본문 → None."""
    from engine.safety.feedback import get_reading_counts
    assert get_reading_counts("never recorded") is None


def test_get_aggregate_stats_basic():
    """누적 통계 — 합계 + 비율."""
    from engine.safety.feedback import record_feedback, get_aggregate_stats
    # 풀이 A: 정합 yes 3 / no 1
    for _ in range(3):
...
def test_get_aggregate_stats_empty():
    """데이터 없을 때 — 비율 None."""
    from engine.safety.feedback import get_aggregate_stats
    s = get_aggregate_stats()
