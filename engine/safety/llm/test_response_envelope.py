"""engine.safety.llm.response_envelope — §7.2.13 envelope 표준 회귀 테스트."""
...
def test_detect_branch_normal():
    from engine.safety.llm.response_envelope import detect_branch, ENVELOPE_NORMAL
    assert detect_branch(_normal_envelope()) == ENVELOPE_NORMAL


def test_detect_branch_cached():
    from engine.safety.llm.response_envelope import detect_branch, ENVELOPE_CACHED
...
def test_detect_branch_crisis():
    from engine.safety.llm.response_envelope import detect_branch, ENVELOPE_CRISIS
    env = _normal_envelope()
...
def test_detect_branch_jailbreak():
    from engine.safety.llm.response_envelope import detect_branch, ENVELOPE_JAILBREAK
    env = _normal_envelope()
...
def test_detect_branch_error_err_prefix():
    from engine.safety.llm.response_envelope import detect_branch, ENVELOPE_ERROR
    env = _normal_envelope()
...
def test_detect_branch_warn_prefix():
    from engine.safety.llm.response_envelope import detect_branch, ENVELOPE_WARN
    env = _normal_envelope()
...
def test_detect_branch_non_dict_returns_empty():
    from engine.safety.llm.response_envelope import detect_branch
...
def test_validate_clean_normal_envelope_passes():
    from engine.safety.llm.response_envelope import validate_envelope
    assert validate_envelope(_normal_envelope()) == []


def test_validate_missing_required_keys():
    from engine.safety.llm.response_envelope import validate_envelope, REQUIRED_KEYS
    env = _normal_envelope()
    del env["text"]
...
def test_is_valid_shortcut():
    from engine.safety.llm.response_envelope import is_valid
...
def test_normalize_fills_missing_required():
    from engine.safety.llm.response_envelope import normalize_envelope
    env = {"text": "test"}
...
def test_audit_all_valid():
    from engine.safety.llm.response_envelope import audit_envelopes
