"""engine.safety.rate_limiter — §7.2.19 rate limiter 회귀 테스트."""
...
def test_initial_acquire_allowed():
    from engine.safety.rate_limiter import RESULT_ALLOWED
    lim = _mk()
...
def test_minute_limit_blocks_11th_call():
    from engine.safety.rate_limiter import RESULT_ALLOWED, RESULT_LIMITED
...
def test_retry_after_sec_reasonable():
    from engine.safety.rate_limiter import RESULT_LIMITED
    lim = _mk(per_minute=2)
    now = 1_700_000_000.0
    lim.acquire("uid1", now=now)
...
def test_alert_allowed_is_p3():
    from engine.safety.rate_limiter import to_alert_payload, RESULT_ALLOWED, RateLimitResult
...
def test_alert_limited_is_p2():
    from engine.safety.rate_limiter import to_alert_payload, RESULT_LIMITED, RateLimitResult
