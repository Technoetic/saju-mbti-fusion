"""engine.safety.kpi_dashboard — §14.11 회귀 테스트."""
...
def test_classify_status_higher_is_better_good():
    from engine.safety.kpi_dashboard import (
...
def test_classify_trend_flat():
    from engine.safety.kpi_dashboard import classify_trend, TREND_FLAT
    # 100 → 101: 1% 변화 → flat
...
def test_classify_trend_up():
    from engine.safety.kpi_dashboard import classify_trend, TREND_UP
    assert classify_trend(current=150, previous=100,
                          higher_is_better=True) == TREND_UP


def test_classify_trend_down():
    from engine.safety.kpi_dashboard import classify_trend, TREND_DOWN
...
def test_build_kpi_known():
    from engine.safety.kpi_dashboard import build_kpi, KPI_P95_LATENCY
...
def test_build_kpi_unknown_raises():
    from engine.safety.kpi_dashboard import build_kpi
    with pytest.raises(ValueError):
        build_kpi("not_a_real_kpi", 0)


def test_build_kpi_with_previous_value():
    from engine.safety.kpi_dashboard import build_kpi, KPI_P95_LATENCY, TREND_UP
    m = build_kpi(KPI_P95_LATENCY, 3000.0, previous_value=2000.0)
    assert m.trend == TREND_UP


def test_build_kpi_persona_pass_rate_bad():
    from engine.safety.kpi_dashboard import build_kpi, KPI_PERSONA_PASS_RATE
...
def test_build_kpi_pii_leak_warn():
    from engine.safety.kpi_dashboard import build_kpi, KPI_PII_LEAK_COUNT
...
def test_build_dashboard_all_good():
    from engine.safety.kpi_dashboard import build_dashboard, STATUS_GOOD
...
def test_build_dashboard_one_bad_makes_overall_bad():
    from engine.safety.kpi_dashboard import build_dashboard, STATUS_BAD
    p = build_dashboard(current_values={
...
def test_build_dashboard_warn_when_only_warn():
    from engine.safety.kpi_dashboard import build_dashboard, STATUS_WARN
    p = build_dashboard(current_values={
...
def test_build_dashboard_unknown_kpi_skipped():
    from engine.safety.kpi_dashboard import build_dashboard
    p = build_dashboard(current_values={
        "p95_latency_ms": 2000,
        "not_real_kpi": 9999,
    })
...
def test_build_dashboard_with_previous_calculates_trend():
    from engine.safety.kpi_dashboard import build_dashboard, TREND_UP
    p = build_dashboard(
...
def test_grafana_json_structure():
    from engine.safety.kpi_dashboard import build_dashboard, to_grafana_json
