"""engine.safety.audit.manual_index — §14.10 회귀 테스트."""
...
def test_all_entries_have_required_fields():
    from engine.safety.audit.manual_index import get_all_entries
...
def test_find_input_guard():
    ...
    assert "engine.safety.llm.jailbreak_defense" in modules
    assert "engine.safety.input_guards.input_sanitizer" in modules
...
def test_find_output_guard():
    ...
    assert len(items) >= 5
    modules = {e.module for e in items}
    assert "engine.safety.llm.persona_self_eval" in modules
    assert "engine.safety.llm.response_pii_leak" in modules
...
def test_find_governance():
    ...
    modules = {e.module for e in items}
    assert "engine.safety.gdpr.consent_screen" in modules
    assert "engine.safety.gdpr.data_governance" in modules
...
def test_find_by_timing_preflight():
    from engine.safety.audit.manual_index import find_by_timing, TIMING_PREFLIGHT
    items = find_by_timing(TIMING_PREFLIGHT)
    assert len(items) >= 3


def test_find_by_timing_postflight():
    from engine.safety.audit.manual_index import find_by_timing, TIMING_POSTFLIGHT
    items = find_by_timing(TIMING_POSTFLIGHT)
    assert len(items) >= 5


def test_find_by_timing_periodic():
    from engine.safety.audit.manual_index import find_by_timing, TIMING_PERIODIC
...
def test_find_p0_severity_modules():
    ...
    from engine.safety.audit.manual_index import find_by_severity
    items = find_by_severity("P0")
    assert len(items) >= 3
    modules = {e.module for e in items}
    assert "engine.safety.crisis.detector" in modules


# ─────────────────────────── 규제 ───────────────────────────

def test_find_gdpr_modules():
    from engine.safety.audit.manual_index import find_by_regulation
    items = find_by_regulation("GDPR")
    assert len(items) >= 3
    modules = {e.module for e in items}
    assert "engine.safety.gdpr.dsr_processor" in modules
...
def test_get_entry_by_module_known():
    from engine.safety.audit.manual_index import get_entry_by_module
    e = get_entry_by_module("engine.safety.llm.jailbreak_defense")
...
def test_format_index_text_has_category_headers():
    from engine.safety.audit.manual_index import format_index_text
...
def test_json_summary_serializable():
    from engine.safety.audit.manual_index import to_json_summary
