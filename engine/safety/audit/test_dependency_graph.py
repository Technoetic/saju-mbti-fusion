"""engine.safety.audit.dependency_graph — §14.13 회귀 테스트."""
...
def test_extract_imports_simple():
    from engine.safety.audit.dependency_graph import _extract_imports_from_source
    src = "from engine.safety.crisis.detector import detect_crisis\n"
    imports = _extract_imports_from_source(src)
    assert "engine.safety.crisis.detector" in imports


def test_extract_imports_multiple():
    from engine.safety.audit.dependency_graph import _extract_imports_from_source
    src = (
        "from engine.safety.crisis.detector import detect_crisis\n"
        "from engine.safety.gdpr.pii import mask_pii\n"
        "import engine.safety.gdpr.legal_notice as ln\n"
    )
    imports = _extract_imports_from_source(src)
    assert "engine.safety.crisis.detector" in imports
    assert "engine.safety.gdpr.pii" in imports
    assert "engine.safety.gdpr.legal_notice" in imports
...
def test_build_graph_has_nodes():
    from engine.safety.audit.dependency_graph import build_graph
    g = build_graph()
    # 최소 30개 운영 모듈
...
def test_no_cycles_in_current_safety_modules():
    ...
    from engine.safety.audit.dependency_graph import build_graph, has_cycles
    g = build_graph()
...
def test_detect_cycles_in_synthetic_edges():
    from engine.safety.audit.dependency_graph import _detect_cycles
    edges = {
        "a": {"b"},
        "b": {"c"},
        "c": {"a"},   # cycle: a→b→c→a
...
def test_in_degree_for_pii():
    ...
    from engine.safety.audit.dependency_graph import build_graph, in_degree
    g = build_graph()
    # 어떤 모듈이든 in_degree >= 0
    assert in_degree(g, "engine.safety.gdpr.pii") >= 0
...
def test_out_degree_for_compliance_report_non_zero():
    ...
    from engine.safety.audit.dependency_graph import build_graph, out_degree
    g = build_graph()
    # 그래프 내부에 있어야
    assert "engine.safety.audit.compliance_report" in g.nodes
    assert out_degree(g, "engine.safety.audit.compliance_report") >= 0


def test_get_dependents_returns_list():
    from engine.safety.audit.dependency_graph import build_graph, get_dependents
    g = build_graph()
    deps = get_dependents(g, "engine.safety.gdpr.pii")
    assert isinstance(deps, list)


def test_get_dependencies_returns_list():
    from engine.safety.audit.dependency_graph import build_graph, get_dependencies
    g = build_graph()
    deps = get_dependencies(g, "engine.safety.audit.compliance_report")
    assert isinstance(deps, list)
...
def test_format_text_includes_summary():
    from engine.safety.audit.dependency_graph import build_graph, format_graph_text
    g = build_graph()
...
def test_to_json_serializable():
    from engine.safety.audit.dependency_graph import build_graph, to_json
    g = build_graph()
    j = to_json(g)
