"""engine.safety.audit.shadow_eval — §7.3.9 페어 셰도우 평가 회귀 테스트."""
...
def test_jaccard_same_text_is_1():
    from engine.safety.audit.shadow_eval import jaccard_overlap
...
def test_pair_identical_is_neutral():
    from engine.safety.audit.shadow_eval import compare_pair, CANDIDATE_NEUTRAL
...
def test_pair_persona_regression_is_worse():
    ...
    from engine.safety.audit.shadow_eval import compare_pair, CANDIDATE_WORSE
...
def test_pair_fast_response_is_better():
    from engine.safety.audit.shadow_eval import compare_pair, CANDIDATE_BETTER
    c = compare_pair(
        control_text=GOOD, candidate_text=GOOD,
...
def test_aggregate_empty_returns_zeros():
    from engine.safety.audit.shadow_eval import aggregate_shadow_results
...
def test_alert_medical_legal_is_p1():
    from engine.safety.audit.shadow_eval import to_alert_payload
    p = to_alert_payload({
