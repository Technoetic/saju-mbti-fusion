"""engine.safety.compliance_report — §14.4 자체 점검 회귀 테스트."""
...
def test_check_item_implemented():
    from engine.safety.compliance_report import check_item
    r = check_item("5.2.4_jailbreak_defense")
    assert r.implemented is True
    assert "detect_jailbreak" in r.evidence
    assert r.module == "engine.safety.jailbreak_defense"
...
def test_format_report_text_includes_coverage():
    from engine.safety.compliance_report import generate_report, format_report_text
    r = generate_report()
