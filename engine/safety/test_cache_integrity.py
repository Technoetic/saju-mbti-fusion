"""engine.safety.cache_integrity — §7.2.16 캐시 무결성 회귀 테스트."""
...
def test_verify_clean_cache_file_passes(tmp_path):
    from engine.safety.cache_integrity import verify_cache_file
...
def test_audit_empty_directory(tmp_path):
    from engine.safety.cache_integrity import audit_cache_directory
...
def test_list_corrupt_files_returns_only_invalid(tmp_path):
    from engine.safety.cache_integrity import list_corrupt_files
    cache = tmp_path / "cache"
    cache.mkdir()
