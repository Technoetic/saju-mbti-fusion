"""engine.safety.changelog_tracker — §14.12 회귀 테스트."""
...
def _fresh_store():
    from engine.safety.changelog_tracker import ChangelogStore
...
def test_record_change_basic():
    from engine.safety.changelog_tracker import CHANGE_ADDED, REASON_IMPROVEMENT
    store = _fresh_store()
...
def test_record_change_with_explicit_timestamp():
    from engine.safety.changelog_tracker import CHANGE_MODIFIED, REASON_REGULATION
    store = _fresh_store()
    e = store.record_change(
...
def test_record_change_with_metadata():
    from engine.safety.changelog_tracker import CHANGE_MODIFIED, REASON_INCIDENT
    store = _fresh_store()
    e = store.record_change(
        change_type=CHANGE_MODIFIED,
        standard_ref="§5.2.7",
...
def test_by_module():
    from engine.safety.changelog_tracker import CHANGE_MODIFIED, REASON_IMPROVEMENT
    store = _fresh_store()
...
def test_breaking_changes_only():
    from engine.safety.changelog_tracker import CHANGE_REMOVED, REASON_DEPRECATION
    store = _fresh_store()
...
def test_default_store_is_singleton():
    from engine.safety.changelog_tracker import get_default_store
    a = get_default_store()
    b = get_default_store()
    assert a is b
