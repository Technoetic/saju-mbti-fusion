"""engine.safety.output_safety_gate — §7.2.21 통합 안전망 회귀 테스트."""
...
def test_empty_response_critical():
    from engine.safety.output_safety_gate import run_safety_gates, VERDICT_CRITICAL
    r = run_safety_gates("")
...
def test_clean_response_all_gates_pass():
    from engine.safety.output_safety_gate import run_safety_gates, VERDICT_CLEAN
    r = run_safety_gates(
        GOOD,
        question="결혼운이 어떤가요?",
        age=30,
...
def test_persona_violation_warn():
    from engine.safety.output_safety_gate import run_safety_gates, VERDICT_WARN
...
def test_too_short_minor():
    ...
    from engine.safety.output_safety_gate import run_safety_gates, VERDICT_MINOR
...
def test_should_fallback_clean_is_false():
    from engine.safety.output_safety_gate import run_safety_gates, should_fallback
    r = run_safety_gates(GOOD, lang="ko")
...
def test_trace_event_fields():
    from engine.safety.output_safety_gate import run_safety_gates, to_trace_event
    r = run_safety_gates(GOOD, lang="ko")
    e = to_trace_event(r)
...
def test_alert_payload_critical_is_p0():
    from engine.safety.output_safety_gate import run_safety_gates, to_alert_payload
    r = run_safety_gates(GOOD + " 010-1234-5678", lang="ko")
