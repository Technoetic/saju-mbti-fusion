"""engine.safety.incident.backup_manifest — §7.3.8 백업·복구 회귀 테스트."""
...
def test_face_reading_manifest_has_all_5_resources():
    from engine.safety.incident.backup_manifest import get_face_reading_manifest
...
def test_governance_audit_is_p0_long_retention():
    """KR/EU 규제 의무 — 7년 보관."""
    from engine.safety.incident.backup_manifest import get_resource_by_id, RECOVERY_PRIORITY_P0
...
def test_slo_logs_rpo_under_5min():
    """SLO 측정은 5분 RPO 필수."""
    from engine.safety.incident.backup_manifest import get_resource_by_id
...
def test_list_backup_targets_excludes_volatile():
    from engine.safety.incident.backup_manifest import list_backup_targets
...
def test_overdue_backups_no_history_all_overdue():
    ...
    from engine.safety.incident.backup_manifest import overdue_backups
...
def test_sha256_is_64_chars(tmp_path):
    from engine.safety.incident.backup_manifest import compute_sha256
    p = tmp_path / "test.bin"
...
def _mk_record(
    ...
    from engine.safety.incident.backup_manifest import BackupRecord
    return BackupRecord(
...
def test_alert_payload_empty_overdue():
    from engine.safety.incident.backup_manifest import to_alert_payload
    p = to_alert_payload([])
