"""engine.safety.gdpr.data_governance — §7.3.3 데이터 거버넌스 회귀 테스트."""
...
def _mk(
    ...
    from engine.safety.gdpr.data_governance import DataProvenance
...
def test_licit_source_full_consent_passes():
    from engine.safety.gdpr.data_governance import validate_provenance
...
def test_is_expired_future_returns_false():
    from engine.safety.gdpr.data_governance import is_expired
...
def test_days_until_expiry_positive_for_future():
    from engine.safety.gdpr.data_governance import days_until_expiry
    p = _mk(retention_days=30, collected_at=_today_iso())
...
def test_is_eligible_for_regression_requires_no_violation_and_not_expired():
    from engine.safety.gdpr.data_governance import is_eligible_for_regression
    # 정상 케이스
...
def test_audit_dataset_aggregates():
    from engine.safety.gdpr.data_governance import audit_dataset
    items = [
...
def test_licit_and_illicit_sources_are_disjoint():
    from engine.safety.gdpr.data_governance import LICIT_SOURCES, ILLICIT_SOURCES
    assert LICIT_SOURCES.isdisjoint(ILLICIT_SOURCES)


def test_required_consent_fields_count():
    from engine.safety.gdpr.data_governance import REQUIRED_CONSENT_FIELDS
