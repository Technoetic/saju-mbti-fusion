...
_COMPAT_SYSTEM = (
    ...
    "결정론적으로 계산된 천간/지지 관계, 오행 흐름, MBTI Socionics 분류, "
    "성명학 음령오행 결합을 바탕으로 한국어 해설을 작성하세요. "
    "단정적 예언 금지 — 점수·등급·'좋은/안 좋은 궁합' 표현 절대 금지. "
    "양면 해석 의무 — 매 섹션마다 강점과 약점·조화와 갈등을 동시 묘사. "
    "경향성·자기이해 조언만. 사용자 인생 결정의 단독 근거 X."
)
...
def explain_compat(
    ...
    user = (
        ...
        f"[지시 — ADR-006·ADR-010·ADR-014·ADR-091·ADR-092 강제]\n"
        ...
        f"  · [ADR-092 균형도] 긍정·부정 표현 비율 1:1 목표. 매 섹션마다 다음 의무:\n"
        f"      (a) 강점/조화/보완 1+ 문장 + (b) 갈등/약점/주의 1+ 문장 동시 작성.\n"
        f"      한 섹션이 긍정만 또는 부정만으로 구성되면 안 됨.\n"
        f"      '암묵적 긍정 단정' (= 부정 표현 없이 긍정만 다수)도 ADR-006 위반.\n"
        + (
