...
MANWOL_SYSTEM = (
    ...
    "[내용 규칙 · 절대 준수]\n"
    "  • 아래 [사주 결정론 데이터] 블록에 있는 값만 근거로 짚어라. 4기둥 · 오행 분포 · 신강 등급 · 격국 · 대운 · 이름 오격 · 관상 지표 · 꿈 원문 · 생활 현황 · MBTI · 고민.\n"
    "  • ★ 데이터 블록에 없는 도메인은 절대 언급 마라. \"타로\" 언급 절대 X. \"자미두수\" 언급 절대 X.\n"
    ...
    "      - 십성 · 신살 — 데이터에 없으면 언급 X\n"
    "      - 자미두수 관련 (자미성/명궁/재백궁 등 12궁·오행국·주성) — 폐기 · 절대 언급 X\n"
    "      - 타로 카드 이름 (바보/황제/여황제/정의/세계/죽음/별 등 22장 전부) — 폐기 · 절대 언급 X · 자기 마음대로 카드 뽑지 마라\n"
    ...
    "  • 관상 문단에서 궁 이름은 얼굴 부위로만 해석 · 운명 매핑 금지.\n"
    "  • 자미두수는 시스템에서 폐기 · 이 단어 · 자미성 · 명궁 배치 · 오행국 등 언급 절대 X.\n\n"
    ...
    "  6. 생활 · MBTI (있으면 · 1문단) — 지금 상황 · 사주 연결.\n"
    "     ※ MBTI 는 payload 에 mbti 값이 있을 때만 언급하라. 없으면 언급 X.\n"
...
def _fmt_extras(payload: dict[str, Any]) -> str:
    ...
    # 자미두수·타로 시스템 폐기 (2026-07-07 사용자 요청). payload 에 왔어도 무시.
    return "\n\n".join(bits)
...
def build_manwol_user_prompt(payload: dict[str, Any]) -> str:
    ...
    if concern and isinstance(concern, str) and concern.strip():
        ...
        parts.append(
            ...
            "  · 사주·관상·꿈 중 최소 2개 도메인 데이터를 연결해서 답변.\n"
    ...
    parts.append(
        ...
        "  구성: 도입 → 사주 골격 → 이름 → 관상 → 꿈 → 생활 → 고민 응답 → 마무리.\n"
        "  자가 점검 5개 (미이행 시 재작성):\n"
        "    1. \"자미두수\" · \"타로\" 언급 없나? (둘 다 시스템 폐기)\n"
