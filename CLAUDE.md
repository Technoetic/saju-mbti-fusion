...
## 3. 외부 보고서 받으면 — 사실성 분리 후 reports/

사용자가 딥리서치·명리학자 자문·문헌 자료를 전달하면:

1. 본문 정독
2. **사실성 3 등급 분리** (ADR-010 의무):
   - 🟢 **팩트** — 자전·문헌·관습으로 검증 가능 → 채택
   - 🟡 **구조** — 검증 불가하나 시스템 설계 명제로 채택 → 객관 라벨로 채택
   - 🔴 **도그마** — 검증 불가 + 위해 가능 (의료·법률 인과 주장) → 폐기
3. **`vault/reports/주제명.md`** 생성 — 3 등급 분리 명시
4. "본 시스템 반영" 섹션에 채택/폐기 구분
5. `vault/reports/INDEX.md` 표에 행 추가
6. 본문화 시작 시 done/ + ADR 동반

### 절대 금지

- ⛔ "⭐⭐⭐⭐⭐ 즉시 본문화 가능"식 무비판 영속화
- ⛔ 출처 검증 없이 "통계적 유의성" 등 권위 수사 그대로 옮기기
- ⛔ 인과적 흉화·물리 발현 표현 사용자 출력에 노출
- ⛔ ADR-006(법률·의료 자문 거절)에 충돌하는 내용 채택

**팩트가 아닌 건 지식이 아니다** (사용자 원칙, 2026-05-17).
