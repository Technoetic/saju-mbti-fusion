...
## 2026-05-17 추가 본문화 — §2 위치별 음절 + 검증된 학술 출처

### A. 위치별 음절 빈도 (보고서 §2 본문 "준·윤으로 끝나는" 명시)

이전 PR은 위치 무관 빈도만 사용. 보고서 §2 본문에 "준·윤으로 끝나는 이름이
자주 관찰" 명시 → 시작/끝 위치별 가중 빈도 추가.

#### 추가 데이터 (같은 70건에서 위치별 추출)
- 남자 끝 음절 1위: **'준' 169점** (보고서 본문 명시와 일치)
- 남자 끝 2위: '우' 50, 3위: '윤' 49 (보고서 본문 명시)
- 여자 끝 1·2위: '아'(84)·'윤'(84) 동률

#### 추가 모듈 함수
- `position_match_score(name, gender)` — 시작/끝 음절 위치별 점수
- `get_top_positional_syllables(gender, position, n)` — 디버깅

#### 검증
- '이준' (남자, 끝='준') → end_score 1.0 ✅
- '괴짜' → combined_score < 0.3 ✅
- 남자 vs 여자 분류 차이 검증 ('준' end_score: male > female)

### B. 검증된 학술 출처 영속화

ADR-016 검증에서 가짜 인용으로 폐기된 출처 → 진짜 출처 references 노트:
- [[../references/korean-phonetic-research]]
- 조성문(2025) "한국인 이름 음운적 특성" (KCI 라이브 검증)
- 신지영(2010) "한국어 사전 표제어 음운 빈도" (KCI 라이브 검증)
- 본 출처는 **현 시점 코드 미활용**. 향후 음운 보강 시 참조용
