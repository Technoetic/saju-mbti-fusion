...
p.on('console', m => { const t = m.text(); if (t.includes('VERIFY_') || t.includes('STEP_')) console.log(t); });
console.log('STEP_1 goto');
...
console.log('STEP_2 click saju card');
await p.evaluate(() => {
...
console.log('STEP_5 click result');
await p.click('#goResultBtn').catch(()=>{});
console.log('STEP_6 wait webtoon');
let found = false;
for (let i=0; i<90; i++) {
  const state = await p.evaluate(() => {
    const c = document.querySelector('.webtoon-container');
    const errEl = document.querySelector('.error, [class*=error]');
    const result = document.querySelector('#resultView');
    return {
      hasWebtoon: !!c,
      slotCount: document.querySelectorAll('.webtoon-slot').length,
      pageCount: document.querySelectorAll('.webtoon-page').length,
      bodyLen: (document.body.innerText || '').length,
      visible: result ? result.offsetParent !== null : false,
    };
  });
  if (i % 5 === 0) console.log('STEP_6 poll '+i+': '+JSON.stringify(state));
  if (state.hasWebtoon) { found = true; break; }
  await p.waitForTimeout(2000);
}
console.log('STEP_7 webtoon found='+found);
if (!found) { await b.close(); process.exit(2); }
